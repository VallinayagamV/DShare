const DShare = artifacts.require('./DShare.sol')

require('chai')
  .use(require('chai-as-promised'))
  .should()

contract('DShare', ([deployer, uploader]) => {
  let dshare

  before(async () => {
    dshare = await DShare.deployed()
  })

  describe('deployment', async () => {
    it('deploys successfully', async () => {
      const address = await dshare.address
    ...
    it('has a name', async () => {
      const name = await dshare.name()
      assert.equal(name, 'DShare')
    })
  })
  ...
  describe('file', async () => {
    ...
    before(async () => {
      result = await dshare.uploadFile(fileHash, fileSize, fileType, fileName, fileDescription, { from: uploader })
      fileCount = await dshare.fileCount()
    })
    ...
    it('upload file', async () => {
      ...
      await dshare.uploadFile('', fileSize, fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have size
      await dshare.uploadFile(fileHash, '', fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;
      
      // FAILURE: File must have type
      await dshare.uploadFile(fileHash, fileSize, '', fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have name
      await dshare.uploadFile(fileHash, fileSize, fileType, '', fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have description
      await dshare.uploadFile(fileHash, fileSize, fileType, fileName, '', { from: uploader }).should.be.rejected;
    })

    //check from Struct
    it('lists file', async () => {
      const file = await dshare.files(fileCount)
